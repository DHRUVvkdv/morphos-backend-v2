...
class TPoseDetector:
    def __init__(self):
        ...
        self.consecutive_frames_required = (
            3  # Frames required to confirm a state change
        )
        ...
        self.CONFIDENCE_THRESHOLD = (
            0.25  # Lowered confidence threshold to be more lenient
        )
    ...
    def check_t_pose(self, keypoints, confidences):
        ...
        left_arm_straightness = min(
            100,
            max(
                0,
                (left_arm_angle - (180 - self.ANGLE_THRESHOLD))
        ...
        left_arm_horizontalness = min(
            100,
            max(
                0,
                (self.HORIZONTAL_TOLERANCE - left_arm_horizontal_diff)
        ...
        feedback["left_arm"]["score"] = (
            left_arm_straightness + left_arm_horizontalness
        ) / 2
        ...
        right_arm_straightness = min(
            100,
            max(
                0,
                (right_arm_angle - (180 - self.ANGLE_THRESHOLD))
                / self.ANGLE_THRESHOLD
                * 100,
            ),
        )
        ...
        right_arm_horizontalness = min(
            100,
            max(
                0,
                (self.HORIZONTAL_TOLERANCE - right_arm_horizontal_diff)
                / self.HORIZONTAL_TOLERANCE
                * 100,
            ),
        )
        ...
        feedback["right_arm"]["score"] = (
            right_arm_straightness + right_arm_horizontalness
        ) / 2
        ...
        spine_angle = self.calculate_angle(
            shoulder_mid, hip_mid, (hip_mid[0], hip_mid[1] + 10)
        )
        ...
        posture_score = min(
            ...
            max(
                0,
                (self.POSTURE_THRESHOLD - deviation_from_upright)
    ...
    def check_bicep_curl_pose(self, keypoints, confidences):
        ...
        if abs(left_curling_score - right_curling_score) < 20:
            ...
            self.primary_arm = (
                "left" if left_curling_score > right_curling_score else "right"
            )
            primary_side = self.primary_arm
        ...
        if self.bicep_curl_state == "down":
            ...
            elbow_score = 100 - min(
                100, max(0, (smoothed_angle - self.angle_threshold_down) * 3)
            )
            ...
            elbow_score = 100 - min(
                100, max(0, (self.angle_threshold_up - smoothed_angle) * 3)
            )
            ...
            position_in_range = (
                smoothed_angle - self.angle_threshold_down
            ) / range_size
        ...
        if primary_side == "left":
            shoulder_hip_dist = np.sqrt(
                (left_shoulder[0] - left_hip[0]) ** 2
                + (left_shoulder[1] - left_hip[1]) ** 2
            )
            shoulder_elbow_dist = np.sqrt((left_shoulder[0] - left_elbow[0]) ** 2)
            arm_position_score = 100 - min(
                100, (shoulder_elbow_dist / shoulder_hip_dist) * 100
            )
            is_arm_close = (
                shoulder_elbow_dist < shoulder_hip_dist * 0.4
            )  # More lenient: 0.3 -> 0.4
        else:
            shoulder_hip_dist = np.sqrt(
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            if results[0].keypoints is not None and len(results[0].keypoints.data) > 0:
                ...
                keypoints = (
                    results[0].keypoints.data[0].cpu().numpy()[:, :2]
                )  # Get x, y coordinates
                confidences = (
                    results[0].keypoints.data[0].cpu().numpy()[:, 2]
                ...
                if self.exercise_mode == "tpose":
                    is_correct_pose, feedback = self.check_t_pose(
                        keypoints, confidences
                    )
                else:  # bicep_curl mode
                    is_correct_pose, feedback = self.check_bicep_curl_pose(
                        keypoints, confidences
                    )
...
async def get_recommendations(
    emotion: str = Query(...), limit: int = Query(3, ge=1, le=10)
):
            ...
            try:
                ...
                if "command" in json_data:
                    ...
                    if command == "get_recommendations":
                        ...
                        # Get recommendations
                        recommendations = (
                            spotify_service.get_recommendations_for_emotion(
                                emotion, limit
                            )
                        )
                        ...
                        await manager.send_json(
                            {"type": "music_recommendations", "data": recommendations},
                            client_id,
                        )
                        continue
                ...
                if "image" in json_data:
                    ...
                    if "emotion" in emotion_result and emotion_result[
                        "emotion"
                    ] not in ["detecting...", "No face detected"]:
                        ...
                        if len(emotion_histories[client_id]) > 20:
                            emotion_histories[client_id] = emotion_histories[client_id][
                                -20:
                            ]

                    # Determine stable emotion
                    stable_emotion = determine_stable_emotion(
                        emotion_histories[client_id]
                    )

                    # Check if we need to get new recommendations
                    if stable_emotion and (
                        client_id not in current_recommendations
                        or len(current_recommendations[client_id].get("tracks", []))
                        == 0
                    ):
                        ...
                        recommendations = current_recommendations.get(
                            client_id, {"tracks": []}
                        )
